...
func Simple() {
	...
	// fmt should NOT be imported since there are no template interpolations
	if strings.Contains(generatedStr, `"fmt"`) {
		t.Error("Generated code should not import fmt when there are no template interpolations")
	}
...
func Display(count: int) {
	...
	// Verify fmt is imported for template interpolation
	if !strings.Contains(generatedStr, `"fmt"`) {
		t.Error("Generated code should import fmt when there is template interpolation")
	}

	// Verify fmt.Sprint is used
	if !strings.Contains(generatedStr, "fmt.Sprint") {
		t.Error("Generated code should use fmt.Sprint for template interpolation")
	}
}
